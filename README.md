...
A production-grade, end-to-end data engineering pipeline built on Microsoft Azure. Ingests raw retail sales data from REST APIs and Azure Blob Storage, transforms it through **Bronze → Silver → Gold** Medallion layers in Databricks, and delivers a Power BI dashboard for business intelligence reporting.
...
┌─────────────────────────────────────────────────────────────────────┐
│                        DATA SOURCES                                  │
│   REST API (Orders)   │   Blob Storage (CSV)   │   SQL Database      │
└───────────┬───────────┴──────────┬─────────────┴──────────┬─────────┘
            │                      │                          │
            └──────────────────────▼──────────────────────────┘
                         Azure Data Factory
...
| CI/CD | Azure DevOps | Automated pipeline deployment |
...
- Source: REST API endpoint returning paginated JSON orders
- Activity: Web Activity (auth) → Copy Activity (REST → ADLS Gen2 Parquet)
- Destination: `raw/orders/YYYY/MM/DD/`

**Incremental load** (daily scheduled):
- Uses `LastModifiedDate` watermark pattern
...
**Bronze** — Raw ingestion, schema enforcement:
```python
# Append-only, no transformations
# Preserve original data with metadata columns
...
```python
# Deduplicate, cast types, handle nulls, standardize
# Output: reliable, queryable data
df_clean = (df_bronze
...
Power BI connects to the Gold Delta tables via Azure Synapse external tables. Key measures use DAX for:
...
```bash
git clone https://github.com/YOUR_USERNAME/azure-data-pipeline.git
...
az datafactory pipeline create \
  ...
  --pipeline @adf/pipelines/pl_ingest_orders.json
```
...
- ✅ **Parameterized pipelines** — Dev/staging/prod environment support
- ✅ **Data quality checks** — Row count validation, null checks, schema drift detection
...
[LinkedIn](https://linkedin.com/in/YOUR_PROFILE) · [Email](mailto:idrismod02@gmail.com)  
